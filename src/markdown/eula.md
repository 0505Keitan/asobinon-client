...
この利用規約（以下，「本規約」といいます。）は、「https://client.markdown.org」で公開されている「アソビノン編集補助サイト」（以下，「当サイト」といいます。）がこのウェブサイト上で提供するサービス（以下，「本サービス」といいます。）の利用条件を定めるものです。登録ユーザーの皆さま（以下，「ユーザー」といいます。）には，本規約に従って，本サービスをご利用いただきます。
...
5. 画像のアップロード後に、自動で画像のデータ解析をし、不適切な画像を検出します。
   ...
   2. APIの結果を`0,1,2,3`の4段階に簡素化し、`1,2,3`は記事への貼り付けを許可しません。
   3. **NSFW判定された画像もURLが発行されていますが、運営以外は入手できません。**
   4. 判定に意義がある場合は、運営に再審議を申し込むことができます。その際はメールを返信に使います。
